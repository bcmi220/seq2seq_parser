...
class Beam(object):
    ...
    def advance(self, word_probs, attn_out):
        ...
        * `attn_out`- attention at the last step
        ...
        cur_offset = len(self.next_ys)
        if cur_offset < self.min_length:
            for k in range(len(word_probs)):
                word_probs[k][self._eos] = -1e20

        # force the ouput to be as long as the input
        if self.fix_length>0:
            if cur_offset <= self.fix_length:
                for k in range(len(word_probs)):
                    word_probs[k][self._eos] = -1e20
            else:
                for k in range(len(word_probs)):
                    for j in range(len(word_probs[k])):
                        if j!=self._eos:
                            word_probs[k][j] = -1e20

        # tree constraints
        if self.tgt_itos is not None and self.fix_length>0:
            for k in range(len(word_probs)):
                assert len(self.tgt_itos) == len(word_probs[k])
                # for j in range(len(word_probs[k])):
                #     label = self.tgt_itos[j]
                #     if label == '<SUBROOT>':
                #         if cur_offset > 1 and cur_offset < self.fix_length:
                #             word_probs[k][j] = -1e20
                    # if label[0] == 'L':
                    #     position = int(label[1:])
                    #     if position > cur_offset:
                    #         word_probs[k][j] = -1e20
                    # if label[0] == 'R':
                    #     position = int(label[1:])
                    #     if position > self.fix_length-cur_offset:
                    #         word_probs[k][j] = -1e20
